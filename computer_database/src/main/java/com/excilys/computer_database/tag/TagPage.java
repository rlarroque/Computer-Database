...
/**
 * This class is used via taglibs in the JSP in order to display the page links at the bottom
 * of the page. It handles diferent parameters such as the current page, the number of page 
 * links to display and the offset (number of items to display)
 * @author excilys
 *
 */
public class TagPage extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		// Define what are the starting and ending pages based on the current page and the number of 
		// links displayed.
		int startingPage = Math.max(currentPage - MAX_PAGES_DISPLAYED / 2, 1);
		int endingPage = MAX_PAGES_DISPLAYED + startingPage;

		// If the ending page if beyond the total amount of pages, then shift the pages.
		...
		try {
			...
			for (int i = startingPage; i < endingPage; i++) {
                if (i == currentPage){
                	// Current page is special because it has no links to another page.
	...
	/**
	 * Construct a link for a page without text or attribute. Text will be the current page number.
	 * @param page current page
	 * @return html link
	 */
	...
	private String constructLink(int page, String text, String attributes) {
		...
		link.append(">")
			.append("<a href=\"")
			.append(uri.replace("##1", String.valueOf(page))
